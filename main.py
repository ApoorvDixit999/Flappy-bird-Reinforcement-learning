...
BASE_IMG = pygame.transform.scale_by(
    surface=pygame.image.load(os.path.join("images", "base.png")), factor=1.5
)
BG_IMG = pygame.transform.scale_by(
    surface=pygame.image.load(os.path.join("images", "bg.png")), factor=1.5
)
...
def draw_window(win, bird, pipes, base, score, gameover):
    ...
    if gameover:
        ...
        win.blit(
            text, (WIN_WIDTH - (WIN_WIDTH / 8) - text.get_width(), WIN_HEIGHT / 2 - 50)
        )
        text = STAT_FONT.render("PRESS 'R' to Restart", 1, (0, 0, 0))
        win.blit(
            text, (WIN_WIDTH - (WIN_WIDTH / 6) - text.get_width(), WIN_HEIGHT / 4 - 50)
        )
